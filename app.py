...
with col2:
    ...
    if selected_gu == "전체":
        st.info(f"""
        **인사이트**  
        2024년 기준 객단가가 가장 높은 자치구는 **{top_gu}**, 평균 객단가는 약 **{top_price:,.0f}원**입니다.  
        ...
        if not selected_row.empty:
            ...
            st.info(f"""
            **인사이트**  
            **{selected_gu}**의 객단가는 약 **{selected_price:,.0f}원**이며, 서울시 25개 자치구 중 **{selected_rank}위**입니다.  
...
with col4:
    ...
    if selected_gu == "전체":
        st.info(f"""
        **인사이트**  
        경제활동인구 1명당 매출이 가장 높은 자치구는 **{top_pop_gu}**이며, 1명당 매출은 약 **{top_pop_sales:,.0f}원**입니다.  
        ...
        if not selected_pop.empty:
            ...
            st.info(f"""
            **인사이트**  
            **{selected_gu}**의 경제활동인구 1명당 매출은 약 **{selected_pop_sales:,.0f}원**이며, 서울시 25개 자치구 중 **{selected_pop_rank}위**입니다.  
